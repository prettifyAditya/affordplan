...
import { useGetAllServicesQuery, useDeleteServiceMutation, useUpdateServiceStatusMutation, useUpdateDisplayOrderMutation } from "@/store/backendSlice/serviceAPISlice";
import ServiceSkeleton from "@/components/backendcomponents/listskeleton";

export default function ManageServiceData() {
  ...
  const { data: serviceData = [], refetch, isLoading } = useGetAllServicesQuery();
  ...
  useEffect(() => {
    if (Array.isArray(serviceData)) {
      ...
      serviceData.forEach(s => {
        newOrders[s.ServiceID] = s.DisplayOrder;
      });
  ...
  const handleDelete = async (ServiceID) => {
    const confirmed = confirm("Are you sure you want to delete this service post?");
    if (!confirmed) return;
    try {
  ...
  const handleDisplayOrderChange = (ServiceID, value) => {
    setDisplayOrders(prev => ({
      ...prev,
      [ServiceID]: value === "" ? "" : parseInt(value, 10)
    }));
  };
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    if (!confirmed) return;
    const data = Object.entries(displayOrders).map(([ServiceID, DisplayOrder]) => ({
      ServiceID,
      DisplayOrder
    }));
    try {
  ...
  const handleApprove = async (ServiceID, currentStatus) => {
    const confirmed = confirm("Are you sure you want to update this status?");
    if (!confirmed) return;
    try {
      const updatedStatus = currentStatus === 1 ? 0 : 1;
      const res = await updateStatus({ ServiceID, ActiveStatus: updatedStatus }).unwrap();
      if (res.success) {
  ...
  const columns = [
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <img
              src={`/OnlineImages/ServiceImages/${row.ServiceImage}`}
              alt={row.ServiceName}
    ...
    },
    {
      ...
      cell: (row) => (
        <input
          type="number"
          value={displayOrders[row.ServiceID] ?? ""}
          onChange={(e) => handleDisplayOrderChange(row.ServiceID, e.target.value)}
        ...
        isLoading ? (
          ...
          <>
            ...
            <button
              ...
              onClick={() => handleApprove(row.ServiceID, row.ActiveStatus)}
            >
              {row.ActiveStatus ? (
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <Link
            href={`/afford-admin/addupd-service?ID=${row.ServiceID}`}
            className="edit-icon"
          >
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <button onClick={() => handleDelete(row.ServiceID)} className="edit-icon">
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <button
            onClick={() => window.open(`/${row.ServiceNameURL}`, "_blank")}
            className="edit-icon"
          >
  ...
  const filteredData = serviceData.filter((item) => {
    ...
    const matchesText = item.ServiceName?.toLowerCase().includes(searchText);
  ...
  const subHeaderComponent = (
    <div className="subheader-container">
      ...
      <div className="colB">
        ...
        <Link
          href={"/afford-admin/addupd-service"}
