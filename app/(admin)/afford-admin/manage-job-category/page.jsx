...
export default function ManageCareerData() {
  ...
  const { data: careerData = [], refetch, isLoading } = useFetchCareersQuery();
  const [deleteCareer] = useDeleteCareerMutation();
  const [updateDisplayOrder] = useUpdateDisplayOrderMutation();
  const [updateCareerStatus] = useUpdateCareerStatusMutation();

  useEffect(() => {
    if (Array.isArray(careerData)) {
      const newOrders = {};
      careerData.forEach(s => {
        newOrders[s.JobCategoryID] = s.DisplayOrder;
      });
  ...
  const handleDelete = async (JobCategoryID) => {
    const confirmed = confirm("Are you sure you want to delete this career?");
    if (!confirmed) return;

    try {
      const res = await deleteCareer(JobCategoryID).unwrap();
      if (res.success) {
  ...
  const handleDisplayOrderChange = (JobCategoryID, value) => {
    setDisplayOrders(prev => ({
      ...prev,
      [JobCategoryID]: value === "" ? "" : parseInt(value, 10)
    }));
  };
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    const data = Object.entries(displayOrders).map(([JobCategoryID, DisplayOrder]) => ({
      JobCategoryID,
      DisplayOrder
    }));
  ...
  const handleApprove = async (JobCategoryID, currentStatus) => {
    const confirmed = confirm("Are you sure you want to update this status?");
    if (!confirmed) return;

    try {
      const updatedStatus = currentStatus === 1 ? 0 : 1;
      const res = await updateCareerStatus({
        JobCategoryID,
  ...
  const columns = [
    {
      name: "Title",
      selector: (row) => row.JobCategoryName,
      sortable: true,
      cell: (row) =>
        isLoading ? (
          <Skeleton width={200} />
    ...
    },
    {
      ...
      cell: (row) => (
        <input
          type="number"
          value={displayOrders[row.JobCategoryID] ?? ""}
          onChange={(e) => handleDisplayOrderChange(row.JobCategoryID, e.target.value)}
        ...
        isLoading ? (
          ...
          <>
            ...
            <button
              ...
              onClick={() => handleApprove(row.JobCategoryID, row.ActiveStatus)}
            >
              {row.ActiveStatus ? (
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <Link
            href={`/afford-admin/addupd-career?ID=${row.JobCategoryID}`}
            className="edit-icon"
          >
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <button onClick={() => handleDelete(row.JobCategoryID)} className="edit-icon">
  ...
  const filteredData = careerData.filter((item) => {
    ...
    const matchesText = item.JobCategoryName?.toLowerCase().includes(searchText);
  ...
  const subHeaderComponent = (
    <div className="subheader-container">
      ...
      <div className="colB">
        ...
        <Link
          href={"/afford-admin/addupd-career"}
