...
export default function ManageMilestoneData() {
  ...
  const handleDelete = async (MilestoneID) => {
    ...
    if (!confirmed) return;
    try {
      const res = await deleteMilestone(MilestoneID).unwrap();
      if (res.success) {
  ...
  const handleApprove = async (MilestoneID, currentStatus) => {
    ...
    try {
      ...
      const res = await updateStatus({ MilestoneID, ActiveStatus: updatedStatus }).unwrap();
      if (res.success) {
  ...
  const columns = [
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <img
              src={`/OnlineImages/MilestoneImages/${row.MilestoneImage}`}
              alt={row.Title}
    ...
    },
    {
      ...
      sortable: true,
      cell: (row) =>
        isLoading ? <Skeleton width={100} /> : <span>{row.PostedDate}</span>,
      width: "140px",
    },
    {
      name: "Status",
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <Link
            href={`/afford-admin/addupd-milestone?ID=${row.MilestoneID}`}
            className="edit-icon"
          >
  ...
  const subHeaderComponent = (
    <div className="subheader-container">
      ...
      <div className="colB">
        <Link
          href={"/afford-admin/addupd-milestone"}
