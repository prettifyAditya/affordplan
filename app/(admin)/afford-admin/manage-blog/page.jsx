...
import { useGetBlogsQuery, useDeleteBlogMutation, useUpdateBlogStatusMutation } from "@/store/backendSlice/blogAPISlice";
...
export default function ManageBlogData() {
  ...
  const { data: blogData = [], refetch, isLoading } = useGetBlogsQuery();
  ...
  const handleApprove = async (BlogID, currentStatus) => {
    ...
    try {
      ...
      const res = await updateStatus({ BlogID, ActiveStatus: updatedStatus }).unwrap();
  ...
  const columns = [
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <img
              src={`/OnlineImages/BlogImages/${row.BlogImage}`}
              alt={row.BlogName}
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <Link
            href={`/afford-admin/addupd-blog?ID=${row.BlogID}`}
            className="edit-icon"
          >
  ...
  const filteredData = blogData.filter((item) => {
    ...
    const matchesText = item.BlogName?.toLowerCase().includes(searchText);
    const matchesOption =
