...
import { useGetAllTeamMembersQuery, useDeleteTeamMemberMutation, useUpdateTeamMemberStatusMutation, useUpdateDisplayOrderMutation } from "@/store/backendSlice/teamAPISlice";
import ServiceSkeleton from "@/components/backendcomponents/listskeleton";

export default function ManageTeamMemberData() {
  ...
  const { data: teamData = [], refetch, isLoading } = useGetAllTeamMembersQuery();
  const [updateDisplayOrder] = useUpdateDisplayOrderMutation();
  const [deleteTeamMember] = useDeleteTeamMemberMutation();
  const [updateStatus] = useUpdateTeamMemberStatusMutation();

  useEffect(() => {
    if (Array.isArray(teamData)) {
      const newOrders = {};
      teamData.forEach(s => {
        newOrders[s.TeamID] = s.DisplayOrder;
      });

      const same =
        JSON.stringify(newOrders) === JSON.stringify(displayOrders);
  ...
  const handleDelete = async (TeamID) => {
    ...
    if (!confirmed) return;
    try {
      const res = await deleteTeamMember(TeamID).unwrap();
      if (res.success) {
  ...
  const handleDisplayOrderChange = (TeamID, value) => {
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    if (!confirmed) return;
    const data = Object.entries(displayOrders).map(([TeamID, DisplayOrder]) => ({
      TeamID,
      DisplayOrder
    }));
    try {
  ...
  const handleApprove = async (TeamID, currentStatus) => {
    const confirmed = confirm("Are you sure you want to update this status?");
    if (!confirmed) return;
    try {
      const updatedStatus = currentStatus === 1 ? 0 : 1;
      const res = await updateStatus({ TeamID, ActiveStatus: updatedStatus }).unwrap();
      if (res.success) {
  ...
  const columns = [
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <img
              src={`/OnlineImages/TeamImages/${row.TeamImage}`}
              alt={row.TeamName}
              className="user-image"
            />
            <div>
              <div style={{ fontWeight: "500" }}>{row.TeamName}</div>
              <div style={{ fontSize: "12px", color: "#666" }}>{row.TeamDesignation}</div>
            </div>
          </div>
        ),
      width: "60%",
    ...
    },
    {
      ...
      cell: (row) => (
        <input
          type="number"
          value={displayOrders[row.TeamID] ?? ""}
          onChange={(e) => handleDisplayOrderChange(row.TeamID, e.target.value)}
        ...
        isLoading ? (
          ...
          <>
            ...
            <button
              ...
              onClick={() => handleApprove(row.TeamID, row.ActiveStatus)}
            >
              {row.ActiveStatus ? (
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <Link
            href={`/afford-admin/addupd-team-member?ID=${row.TeamID}`}
            className="edit-icon"
          >
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <button onClick={() => handleDelete(row.TeamID)} className="edit-icon">
  ...
  const filteredData = teamData.filter((item) => {
    ...
    const matchesTeamType =
      !teamTypeFilter || item.TeamType === teamTypeFilter;
    return matchesText && matchesStatus && matchesTeamType;
  });
    ...
    <div className="subheader-container">
      <div className="colA">
        ...
        >
          ...
          {[...new Set(teamData.map(t => t.TeamType))].map((type, i) => (
      ...
      <div className="colB">
        ...
        <Link
          href={"/afford-admin/addupd-team-member"}
