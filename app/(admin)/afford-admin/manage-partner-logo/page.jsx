...
import { useGetPartnerLogosQuery, useUpdatePartnerLogoStatusMutation, useDeletePartnerLogoMutation, useUpdateDisplayOrderMutation } from "@/store/backendSlice/partnerLogoAPISlice";
...
export default function ManagePartnerLogoData() {
    ...
    const { data: partnerLogoData = [], refetch, isLoading } = useGetPartnerLogosQuery();
    ...
    const handleDisplayOrderChange = (PartnerLogoID, value) => {
        setDisplayOrders(prev => ({
            ...prev,
            [PartnerLogoID]: value === "" ? "" : parseInt(value, 10)
        }));
    };
    ...
    useEffect(() => {
        if (Array.isArray(partnerLogoData)) {
            const initialOrders = {};
            partnerLogoData.forEach(s => {
                initialOrders[s.PartnerLogoID] = s.DisplayOrder;
            });
    ...
    const handleUpdateDisplayOrder = async () => {
        ...
        if (!confirmed) return;
        const data = Object.entries(displayOrders).map(([PartnerLogoID, DisplayOrder]) => ({
            PartnerLogoID,
            DisplayOrder
        }));
        try {
    ...
    const columns = [
        {
            name: "Partner Logo",
            selector: (row) => row.PartnerLogoImage,
            sortable: true,
            cell: (row) =>
                isLoading ? (
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        {row.PartnerLogoImage && !imageErrors[row.PartnerLogoID] ? (
                            <img
                                src={`/OnlineImages/PartnerLogoImages/${row.PartnerLogoImage}`}
                                alt={row.PartnerLogoImage}
                                className="user-image"
                                onError={() => {
                                    setImageErrors(prev => ({
                                        ...prev,
                                        [row.PartnerLogoID]: true
                                    }));
                                }}
                            />
                        ) : (
                            <div className="user-image-none">
                                {row.PartnerLogoImage ? row.PartnerLogoImage.split(".")[0] : row.SerialNo}
                            </div>
                        )}
                    </div>
                ),
        ...
        },
        {
            ...
            cell: (row) => (
                <input
                    type="number"
                    value={displayOrders[row.PartnerLogoID] !== undefined ? displayOrders[row.PartnerLogoID] : ""}
        ...
        },
        {
            ...
            cell: (row) =>
                isLoading ? (
                    ...
                    <Link
                        href={`/afford-admin/addupd-partner-logo?ID=${row.PartnerLogoID}`}
                        className="edit-icon"
                    >
    ...
    const filteredData = partnerLogoData.filter(item => {
    ...
    const subHeaderComponent = (
        <div className="subheader-container">
            ...
            <div className="colB">
                ...
                <Link
                    href={"/afford-admin/addupd-partner-logo"}
    ...
    return (
        <main>
            <DataTable
                title="Manage Partner Logo Data"
                columns={columns}
                data={filteredData}
                striped
                pagination
